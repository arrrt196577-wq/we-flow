...
子 agent **只有两种类型**，按能力与影响范围（blast radius）划分，而**不按业务角色**铺类型：

- `search_agent`（**研究型**）：只读研究。web 搜索 / 抓取 + workspace 文件检索，
  不写文件、不执行命令、不再委派。可并行、对不可信输入安全。
- `implement_agent`（**执行型**）：读 + 写（`apply_patch / write_file`）+
  受控命令（`run_command` 白名单）。带写 / 执行影响范围，按权限收口。

这对应 DeerFlow 的两个内建 subagent（`general-purpose` + `bash`），
但我们用更严格的「只读 vs 可写」边界作为分界线，替代 DeerFlow 的全工具 general-purpose。

> **不新增 `report_agent` / `plan_agent` 等角色化子 agent。**
> 规划与报告写作由 **lead 编排 + skills（领域模板 / 方法论，按需加载）** 承担；
> 新领域能力 = 加 skill 或配置化 custom agent，而不是加硬编码角色 agent。
...
| 已有 agent | `lead_agent`（全工具协调）、`search_agent`（**研究型**：只读研究，六段式输出 + `[citation:Title](URL)`）、`implement_agent`（**执行型**：写 / 命令，当前占位） |
...
1. 缺**计划层**（research plan / 大纲），目前由 lead 临时决定是否委派；**不另立 plan_agent**，应由 lead 编排 + plan skill 承担。
2. 缺**报告写作能力**（结构化长报告），目前靠 lead 顺手拼、质量不可控；**不另立 report_agent**，应由 lead 综合 + report skill 承担。
...
### A1. 计划层（lead + plan skill，不建 plan_agent）

- **不新增 `plan_agent`**。规划由 **lead 编排**完成：输入主题 →
  输出结构化大纲（章节 + 每章子问题 + 检索关键词）。
- 大纲生成方法论沉淀为 **plan skill**（按需加载），让后续研究**按章节并行委派**，
  而非漫游式 ReAct。
- 对标 DeerFlow：planning 是 lead 的职责 + skill，不是独立 subagent。

### A2. 强化研究执行

- `search_agent`（**研究型**）已是只读研究器，重点补：**多来源 + 去重 + 来源质量**。
...
### A3. 报告写作（lead + report skill，不建 report_agent）

- **不新增 `report_agent`**。报告综合由 **lead** 完成：吃“大纲 + 各章节研究结果 +
  证据池”，产出长 markdown，每个论断挂引用。
- 报告结构 / 引用规范沉淀为 **report skill**（按需加载）；落盘用轻量「写产物」
  工具（或执行型 `implement_agent` 的写能力）。
- 这是“闭环”的临门一脚——补的是**能力（report skill + 写产物）**，而不是一个新角色 agent。
...
- **B1 落地执行型 `implement_agent`**：按 [next-26-6-17.md](./next-26-6-17.md) 设计——
  只读工具 + `apply_patch / write_file` + `run_command`（allowlist：`rg / mvn test / git status` 等），
  先权限边界后开工具。这是两种子 agent 类型中「执行型」的真正落地（A3 报告落盘用的轻量写产物工具是其前置子集）。
...
- **B5 规划能力泛化**：把 A1 的报告规划（lead 编排 + plan skill）泛化成通用任务规划，支撑非报告类多步任务；**仍不引入独立 planner agent**。
- **B6 技能 / 领域模板体系**：把 A8 报告模板、A1 plan skill、A3 report skill 统一成 “skills” 体系，不同领域 = 不同技能包——这是替代角色化 agent 的主路径。
...
- **可观测与成本**：`runs` 已有 token 字段，补 tracing 与分段计费（lead vs 研究型 vs 执行型 子 agent）。
...
2. **加 report skill + 写产物 + artifact 存储**（不建 report_agent），打通 `主题 → 大纲(lead, A1) → 研究(A2) → 报告 markdown(A3/A5)` 一条直线，先不追求完美质量。
...
### 第 2 周：计划层（lead + plan skill，不建 plan_agent）

**Day 8 — 设计 plan 数据结构**
- [ ] 在 `we-flow-core` 定义 `ResearchPlan`（章节 + 子问题 + 检索关键词）
- [ ] 确认形态：**lead 内规划步骤 + plan skill**，不引入 `plan_agent`

**Day 9 — plan skill + lead prompt**
- [ ] 写第一个 **plan skill**（主题 → 结构化大纲的方法论 / 模板）
- [ ] lead prompt 接入：复杂调研先按 plan skill 生成大纲
...
**Day 11 — lead 编排接入**
- [ ] lead 按大纲**并行 `delegate_task(search_agent)`**
...
**Day 18 — report skill + 写产物能力（A3）**
- [ ] 写 **report skill**（报告结构 + 引用规范，按需加载）
- [ ] 加轻量「写产物」工具（lead / 执行型可调用），落盘 markdown

**Day 19 — lead 报告综合接入编排**
- [ ] 研究完成 → lead 按 report skill 综合大纲 + 证据池 → 长 markdown
- [ ] 论断挂引用 id
